    ...
    for epoch in tqdm(range(epochs)):
        ...
        if early_stopping and epoch > early_stopping:
            if min(history['val_loss']) not in history['val_loss'][-early_stopping:]:
                print(f'Early stopping. Validation loss has not improved for last {early_stopping} epochs.')
                break
...
if __name__ == '__main__':
    ...
    net = MultiClassClassifier(class_number=config.NUM_CLASSES, train_backbone=False).to(config.DEVICE)
    loss_function = torch.nn.CrossEntropyLoss()
    opt = torch.optim.Adam(net.parameters(), lr=config.LR)

    perform_learning(epochs=config.EPOCHS,
